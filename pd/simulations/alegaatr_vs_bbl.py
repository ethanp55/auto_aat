...
compressed_rewards_df.to_csv(f'../../analysis/{str(prisoners_game)}/algaater_vs_bbl_algaater{file_modifier}.csv')

compressed_rewards_df_opp = pd.DataFrame(vals_2, columns=['BBL'])
compressed_rewards_df_opp.to_csv(f'../../analysis/{str(prisoners_game)}/algaater_vs_bbl_bbl{file_modifier}.csv')
...
plt.title('AlegAATr vs. BBL')
plt.xlabel('Round #')
plt.ylabel('Rewards ($)')
plt.legend(loc="upper left")
plt.savefig(f'../simulations/{str(prisoners_game)}/algaater_vs_bbl{file_modifier}.png', bbox_inches='tight')
plt.clf()

...
pred_model = load_model(f'../aat/auto_aat_tuned/{str(game)}_tuned_model.keras')
pred_scaler = pickle.load(open(f'../aat/auto_aat_tuned/{str(game)}_tuned_scaler.pickle', 'rb'))
        ...
        for expert_key in experts.keys():
            ...
            for opponent_key in opponents.keys():
                ...
                for round_num in range(n_rounds):
                    ...
                    state_input_scaled = pred_scaler.transform(np.array(state_input).reshape(1, -1))
                    new_assumptions = pred_model.auto_aat_model((np.array(g_description).reshape(1, -1),
...
for expert_key, data in training_data.items():
    with open(data_dir + expert_key + '_training_data_auto_tuned.pickle', 'wb') as f:
        pickle.dump(data, f)
...
for expert_key, _ in experts.items():
    training_data_file = expert_key + '_training_data_auto_tuned.pickle'
    ...
    trained_knn_file = expert_key + '_trained_knn_aat_auto_tuned.pickle'
    trained_knn_scaler_file = expert_key + '_trained_knn_scaler_aat_auto_tuned.pickle'

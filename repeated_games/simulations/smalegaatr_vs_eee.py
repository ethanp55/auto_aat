...
vector_file = f'../../analysis/{str(game)}_vectors/SMAlegAATr_eee.csv'
...
for epoch in range(1, n_epochs + 1):
    ...
    reward_map = {smalegaatr.name: 0, eee.name: 0}
    ...
    for round_num in range(n_rounds):
        ...
        key_agent_map = {smalegaatr.name: smalegaatr, eee.name: eee} if smalegaatr_idx == P1 else \
            {eee.name: eee, smalegaatr.name: smalegaatr}
...
compressed_rewards_df.to_csv(f'../../analysis/{str(game)}/smalegaatr_vs_eee_smalegaatr.csv')

compressed_rewards_df_opp = pd.DataFrame(vals_2, columns=['BBL'])
compressed_rewards_df_opp.to_csv(f'../../analysis/{str(game)}/smalegaatr_vs_eee_eee.csv')
...
plt.title('SMAlegAATr vs. EEE')
plt.xlabel('Round #')
plt.ylabel('Rewards ($)')
plt.legend(loc="upper left")
plt.savefig(f'../simulations/{str(game)}/smalegaatr_vs_eee.png', bbox_inches='tight')
plt.clf()

from forex.strategies.qalegaatr import AutoAATHead, AutoAATWithHead
...
from utils.utils import FOREX_E_DESCRIPTION, NETWORK_NAME


generator_names = ['BarMovement', 'BeepBoop', 'BollingerBands', 'Choc', 'KeltnerChannels', 'MACrossover', 'MACD',
...
for currency_pair in CURRENCY_PAIRS:
    for time_frame in TIME_FRAMES:
        for year in YEARS[:-1]:
            states, labels = None, None
            pair_time_year_str = f'{currency_pair}_{time_frame}_{year}'
            print(f'TUNING FOR {pair_time_year_str}')

            for file in os.listdir(training_data_folder):
                if 'states' not in file and 'trade_amounts' not in file:
                    continue
                if pair_time_year_str not in file:
                    continue

                # data = np.genfromtxt(f'{training_data_folder}{file}', delimiter=',', skip_header=0)
                data = pickle.load(open(f'{training_data_folder}{file}', 'rb'))
                ...
                if is_state:
                    ...
                    generator_name = file.split('_')[0]
                    generator_idx = generator_names.index(generator_name)
                    expanded_array = np.zeros((data.shape[0], len(generator_names)))
            ...
            g_text_train, e_text_train = \
                np.array([g_description] * len(states_train)), np.array([FOREX_E_DESCRIPTION] * len(states_train))
            states_val, y_val = states[val_indices, :], labels[val_indices,]
            g_text_val, e_text_val = \
                np.array([g_description] * len(states_val)), np.array([FOREX_E_DESCRIPTION] * len(states_val))
            ...
            with open(f'../aat/auto_aat_tuned/{pair_time_year_str}_tuned_scaler.pickle', 'wb') as f:
            ...
            for epoch in range(N_EPOCHS):
                ...
                if val_mse < best_val_mse:
                    ...
                    # Save the network
                    model.save(f'../aat/auto_aat_tuned/{pair_time_year_str}_tuned_model.keras')

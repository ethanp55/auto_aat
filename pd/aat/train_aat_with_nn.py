import random
from pd.prisoners_dilemma import PrisonersDilemma, baselines, ACTIONS
from pd.agents.folk_egal import FolkEgalAgent, FolkEgalPunishAgent
from pd.agents.minimax_q import MinimaxAgent
from pd.agents.alegaatr import ESTIMATES_LOOKBACK, TRAIN_RANDOM_PROB, TRAIN_RANDOM_N_ROUNDS_RATIOS, \
    distance_func, AssumptionChecker, Assumptions
from pd.agents.cfr import CFRAgent
from utils.utils import P1, P2, PAD_VAL, PRISONERS_E_DESCRIPTION, NETWORK_NAME
import numpy as np
        ...
        for expert_key in experts.keys():
            ...
            for opponent_key in opponents.keys():
                ...
                for round_num in range(n_rounds):
                    ...
                    state_input_scaled = state_scaler.scale(np.array(state_input).reshape(1, -1))
                    new_assumptions = assumption_pred_model((np.array(g_description).reshape(1, -1),
                                                             np.array(PRISONERS_E_DESCRIPTION).reshape(1, -1),
                                                             state_input_scaled)).numpy()
                    new_assumptions = new_assumptions[0, :7]

                    prev_i.append(new_assumptions[0])
                    prev_e.append(new_assumptions[1])
                    prev_v.append(new_assumptions[2])
                    prev_f.append(new_assumptions[3])
                    prev_b.append(new_assumptions[4])
                    prev_p.append(new_assumptions[5])
                    prev_u.append(new_assumptions[6])
...
for expert_key, data in training_data.items():
    with open(data_dir + expert_key + '_training_data_auto.pickle', 'wb') as f:
        pickle.dump(data, f)
...
for expert_key, _ in experts.items():
    training_data_file = expert_key + '_training_data_auto.pickle'
    ...
    trained_knn_file = expert_key + '_trained_knn_aat_auto.pickle'
    trained_knn_scaler_file = expert_key + '_trained_knn_scaler_aat_auto.pickle'

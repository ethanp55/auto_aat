from forex.strategies.smalegaatr import AutoAATHead, AutoAATWithHead
...
from utils.utils import FOREX_E_DESCRIPTION, FOREX_G_DESCRIPTIONS, NETWORK_NAME
    ...
    for time_frame in TIME_FRAMES:
        for year in YEARS[:-1]:
            ...
            for file in os.listdir(training_data_folder):
                ...
                if is_state:
                    name = file.split('_')[0]
                    g_text.extend([FOREX_G_DESCRIPTIONS[name]] * data.shape[0])
            ...
            g_text_train, e_text_train = g_text[train_indices, ], np.array([FOREX_E_DESCRIPTION] * len(states_train))
            states_val, y_val = states[val_indices, :], labels[val_indices,]
            g_text_val, e_text_val = g_text[val_indices], np.array([FOREX_E_DESCRIPTION] * len(states_val))

...
from pd.agents.alegaatr import AlegAATr, ESTIMATES_LOOKBACK, Assumptions
from pd.agents.eee import EEE
...
for epoch in range(1, n_epochs + 1):
    ...
    eee_experts = AlegAATr.create_aat_experts(prisoners_game, eee_idx)

    algaater = AlegAATr('Algaater', prisoners_game, algaater_idx, baselines, use_auto_aat=use_auto_aat)
...
compressed_rewards_df.to_csv(f'../../analysis/{str(prisoners_game)}/algaater_vs_eee_algaater{file_modifier}.csv')

compressed_rewards_df_opp = pd.DataFrame(vals_2, columns=['EEE'])
compressed_rewards_df_opp.to_csv(f'../../analysis/{str(prisoners_game)}/algaater_vs_eee_eee{file_modifier}.csv')
...
plt.savefig(f'../simulations/{str(prisoners_game)}/algaater_vs_eee{file_modifier}.png', bbox_inches='tight')
plt.clf()

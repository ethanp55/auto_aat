...
from repeated_games.agents.bbl import BBL
...
vector_file = f'../../analysis/{str(game)}_vectors/SMAlegAATr_bbl.csv'
...
for epoch in range(1, n_epochs + 1):
    ...
    for round_num in range(n_rounds):
        ...
        key_agent_map = {smalegaatr.name: smalegaatr, bbl.name: bbl} if smalegaatr_idx == P1 else \
            {bbl.name: bbl, smalegaatr.name: smalegaatr}
...
compressed_rewards_df.to_csv(f'../../analysis/{str(game)}/smalegaatr_vs_bbl_smalegaatr.csv')

compressed_rewards_df_opp = pd.DataFrame(vals_2, columns=['BBL'])
compressed_rewards_df_opp.to_csv(f'../../analysis/{str(game)}/smalegaatr_vs_bbl_bbl.csv')
...
plt.title('SMAlegAATr vs. BBL')
plt.xlabel('Round #')
plt.ylabel('Rewards ($)')
plt.legend(loc="upper left")
plt.savefig(f'../simulations/{str(game)}/smalegaatr_vs_bbl.png', bbox_inches='tight')
plt.clf()

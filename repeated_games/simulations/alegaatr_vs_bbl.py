...
vector_file = f'../../analysis/{str(game)}_vectors/Alegaatr1_bbl{file_modifier}.csv'
...
compressed_rewards_df.to_csv(f'../../analysis/{str(game)}/algaater_vs_bbl_algaater{file_modifier}.csv')

compressed_rewards_df_opp = pd.DataFrame(vals_2, columns=['BBL'])
compressed_rewards_df_opp.to_csv(f'../../analysis/{str(game)}/algaater_vs_bbl_bbl{file_modifier}.csv')
...
plt.savefig(f'../simulations/{str(game)}/algaater_vs_bbl{file_modifier}.png', bbox_inches='tight')
plt.clf()

...
from networks.state_only import StateOnly
import numpy as np
import pickle
...
# Create the model
model = StateOnly(mask_value=PAD_VAL, state_output_dim=128, use_pos_encodings=True, dense_dim=128, dropout_rate=0.1)
...
for epoch in range(N_EPOCHS):
    ...
    for i in range(n_batches):
        ...
        with tf.GradientTape() as tape:
            predictions = model(curr_state, training=True)
    ...
    for i in range(n_val_batches):
        ...
        val_predictions = model(curr_state)
